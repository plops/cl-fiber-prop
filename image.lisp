...
(defun convert-u16-u8 (a &key (scale .1))
  (declare (type (array (unsigned-byte 16) 2) a)
	   (values (simple-array (unsigned-byte 8) 2) &optional))
  (let ((b (make-array (array-dimensions a)
		       :element-type '(unsigned-byte 8))))
    (destructuring-bind (h w) (array-dimensions a)
      (dotimes (i w)
	(dotimes (j h)
	  (setf (aref b j i) (min 255 (max 0 (floor (* scale (aref a j i)))))))))
    b))

(defun convert-u16-cdf (a)
  (declare (type (simple-array (unsigned-byte 16) 2) a)
	   (optimize (speed 3))
  ...
  (let* ((b (make-array (array-dimensions a)
    ...
    (destructuring-bind (h w) (array-dimensions a)
      ...
      (dotimes (i w)
	(dotimes (j h)
	  (setf (aref b j i) (complex (* 1d0 (aref a j i)))))))
    b))
